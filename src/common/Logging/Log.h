...
namespace Warhead
{
    ...
    public:
        ...
        inline void OutMessage(std::string_view filter, LogLevel const level, std::string_view file, std::size_t line, std::string_view function, Warhead::FormatString<Args...> fmt, Args&&... args)
        {
            _OutMessage(filter, level, file, line, function, StringFormat(fmt, std::forward<Args>(args)...));
        }
